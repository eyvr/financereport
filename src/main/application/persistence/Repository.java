...
public class Repository implements model.persistence.Repository {
    ...
    public List<Transaction> getTransactionsForMonth(Month month) throws SQLException {
        ...
        PreparedStatement stmt = this.connection.prepareStatement(
                "select title, date, type, amount from transactions2 where date >= ? and date < ?"
        );
    ...
    public List<TransactionType> getTypes() throws SQLException {
        ...
        PreparedStatement stmt = this.connection.prepareStatement(
                "select type from transactions2 group by type"
        );
        ResultSet rs = stmt.executeQuery();
